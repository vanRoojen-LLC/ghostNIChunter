...
foreach ($targetGroupName in $targetGroupNames) {
    foreach ($vmStatus in @(Get-AzVM -ResourceGroupName $targetGroupName -Status -ErrorAction Stop)) {
        $powerStatus = $vmStatus.Statuses | Where-Object { $_.Code -like 'PowerState/*' } | Select-Object -Last 1
        $vmPowerStateById["$targetGroupName/$($vmStatus.Name)"] = if ($powerStatus) { [string]$powerStatus.DisplayStatus } else { 'Unknown' }
    }
}
