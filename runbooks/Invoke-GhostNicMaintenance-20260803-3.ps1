...
param(
    ...
    [bool]$RequireProblemCode45 = $true
)
...
if ($ghosted.Count -gt $MaximumCandidateCount) {
    ...
    foreach ($candidate in $ghosted) {
        ...
        if (Get-PnpDevice -InstanceId $candidate.PnpDeviceId -ErrorAction SilentlyContinue) {
            if (-not $RequireProblemCode45 -or $candidate.ProblemCode -eq 45) {
                $method = 'pnputil'
...
foreach ($vmResourceId in $TargetVmResourceIds) {
    ...
    try {
        $result = Invoke-AzVMRunCommand `
            ...
            -Parameter @{ Operation = $Operation; ConfirmRemoval = [bool]$ConfirmRemoval; MaximumCandidateCount = [int]$MaximumCandidateCount; PnpCleanWaitSeconds = [int]$PnpCleanWaitSeconds; RequireProblemCode45 = [bool]$RequireProblemCode45 } `
            -ErrorAction Stop
